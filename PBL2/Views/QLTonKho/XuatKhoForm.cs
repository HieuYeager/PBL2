...
namespace PBL2.Views.QLTonKho
{
    public partial class XuatKhoForm : Form
    {
        ...
        public XuatKhoForm(AccountModel account)
        {
            InitializeComponent();
        ...
        private void XacNhan_Click(object sender, EventArgs e)
        {
            ...
            //check so luon
            int soLuongHienCo = Convert.ToInt32(MySQL_DB.Instance.ExecuteScalar($"SELECT SoLuong FROM nguyenlieu WHERE MaNguyenLieu = '{maNL}'"));
            if (soLuongHienCo < soLuongNhap)
            {
                MessageBox.Show("Số lượng trong kho không đủ để xuất!");
                return;
            }
            ...
            string query = $"INSERT INTO lichsutonkho (MaNguyenLieu, MaNV, HoatDong, SoLuong, GhiChu, Ngay) " +
                           $"VALUES ('{maNL}', '{maNV}', 'Xuất kho', {soLuongNhap}, '{ghiChu.Replace("'", "''")}', NOW())";
